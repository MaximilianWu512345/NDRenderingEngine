//incomplete - not neede maybe
import java.awt.Color;
import java.util.*;
import java.lang.*;
/** Camera class */
public class CameraRastorizationV1 implements Camera{
...
*/
   public CameraRastorizationV1 (Point position, Vector v, int w, int h){
      ...
      for(Simplex s: simplexes){
         ...
         for(int i = 0; i<result.length; i++){
            for(int j = 0; j<result[i].length; j++){
               ...
               if(s.isWithin(p)){
                  result[i][j] = s.getTexture().getColor(null);
